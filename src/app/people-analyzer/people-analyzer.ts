...
@Component({
  ...
  styles: [`
    ...
    .glance-row { display: flex; gap: 16px; flex-wrap: wrap; }
    .glance-box {
      flex: 1 1 0; min-width: 160px; background: #fff; border-radius: 12px;
      box-shadow: 0 2px 8px rgba(0,0,0,.08); padding: 20px 16px; text-align: center;
    }
    .glance-header {
      font-size: 1rem; font-weight: 700; color: #7c5cbf;
      text-transform: uppercase; letter-spacing: .05em; margin-bottom: 8px;
    }
    .glance-avg { font-size: 3rem; font-weight: 800; color: #4a4a6a; line-height: 1; margin-bottom: 10px; }
    .glance-names { display: flex; flex-direction: column; gap: 2px; font-size: .88rem; font-weight: 600; }
    .glance-names.red span { color: #d32f2f; }
    .glance-names.green span { color: #2e7d32; }
...
export class PeopleAnalyzer {
  ...
  top(key: ScoreKey): string[] {
    return this.people.filter(p => p[key] === 9 || p[key] === 10).map(p => p.name);
  }
